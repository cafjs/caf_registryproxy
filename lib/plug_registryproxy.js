...
exports.newInstance = function($, spec, cb) {
    ...
    try {
        ...
        var dynConfig  = $._.$.paas &&
            $._.$.paas.getServiceConfig('registry') || null;

        // port > 0
        var port = (dynConfig && dynConfig.port) || spec.env.port;
        assert.equal(typeof port, 'number', 'Port is not a number');

        var hostname = (dynConfig && dynConfig.hostname) || spec.env.hostname;
        assert.equal(typeof hostname, 'string', 'Hostname is not a string');
        ...
        var target = 'https://' + hostname + ':' + port;
